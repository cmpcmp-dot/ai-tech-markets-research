#!/usr/bin/env Rscript
# ─────────────────────────────────────────────────────────────────────────────
# qwi/02_build_qwi.R — pure transform. Offline, deterministic, asserts loudly.
#
# Turns 102 per-state LEHD bulk files into one national NAICS3 × age × quarter
# panel of employment and worker flows, and validates it against the published
# national aggregate.
#
# Reads:   qwi/output/raw/*.csv.gz         (from 01_fetch_qwi.R)
#          qwi/labels/*.csv
# Writes:  qwi/output/qwi_us_sector.rds            published national, sector level
#          qwi/output/qwi_state_naics3_slim.rds    the slim state panel, kept on disk
#          qwi/output/qwi_naics3_age_quarter.rds   the analysis frame
#          qwi/output/qwi_coverage.csv             per cell: states in, share of national
#          qwi/output/qwi_meta.json                vintage, latest quarter, assertions
#
# ── Two design decisions that matter ─────────────────────────────────────────
#
# 1. STATUS FLAGS. LEHD ships a flag per measure (label_flags.csv):
...
#    spurious jumps: a state whose cell is suppressed in 2019 and released in
#    2024 adds a level shift that looks like growth. So for each
#    (industry × agegrp × measure) we keep only states that report the cell in
#    EVERY quarter of the analysis window, and publish the resulting coverage
#    against the true national total. A consistent aggregate of 43 states beats
#    an inconsistent aggregate of 51.
# ─────────────────────────────────────────────────────────────────────────────
...
SECTOR_TOL <- 0.02    # state-sum vs published national, sector level: 2% max gap
LAG_TOL    <- 4L      # a state more than 4 quarters behind the pack is excluded

# Measures kept, with the status-flag column that governs each.
...
KEYCOLS <- c("geography", "industry", "agegrp", "year", "quarter")
...
read_qwi <- function(path) {
  ...
  # Collapse the per-measure flags to one distorted indicator per row, so the
  # panel carries the caveat without carrying 13 extra columns.
...
nat <- read_qwi(out("raw", "qwi_us_sa_f_gn_ns_op_u.csv.gz"))
...
saveRDS(nat, out("qwi_us_sector.rds"), compress = "xz")
...
st_ns <- rbindlist(lapply(STATES, function(s)
  read_qwi(out("raw", sprintf("qwi_%s_sa_f_gs_ns_op_u.csv.gz", s)))))
say("  %s rows", format(nrow(st_ns), big.mark = ","))

# The end quarter differs by state: LEHD publishes on each state's own
# data-sharing schedule. In this vintage Alaska stops at 2016Q2 and Michigan at
# 2021Q4 while the other 49 reach 2025Q3. Letting the slowest state set the
# window would cost nine years and make the whole exercise pointless, so states
# more than LAG_TOL quarters behind the pack are excluded outright and the cost
# is measured against the published national total rather than waved away.
...
# reported. V1 is checked there, on the full set, because that is the only
# window where "sum of states == published national" is the right expectation.
...
# the era when all 51 still reported. Expectation: the sum reproduces the
# published national essentially exactly.
...
# V1b — the price of excluding the laggards. Not a pass/fail; a number to publish.
...
st_n3 <- rbindlist(lapply(STATES, function(s)
  read_qwi(out("raw", sprintf("qwi_%s_sa_f_gs_n3_op_u.csv.gz", s)))))
st_n3 <- st_n3[ind_level == "3" & !geography %in% LAGGARDS &
               qidx(year * 10L + quarter) <= qidx(END_YQ)]
say("  %s rows, %d subsectors", format(nrow(st_n3), big.mark = ","), st_n3[, uniqueN(industry)])
saveRDS(st_n3, out("qwi_state_naics3_slim.rds"), compress = "xz")
say("  slim state panel kept at %s (the state x industry x age design needs no re-download)",
    basename(out("qwi_state_naics3_slim.rds")))
...
# Denominator is EmpS (full-quarter employment), matching QWI's own published
# rate definitions. Note this means hire rates can legitimately exceed 100%:
# HirA counts every hire including short spells, while EmpS by construction
# excludes anyone who did not last a full quarter. In small seasonal cells
# (14-18 year olds in performing arts, say) that ratio is genuinely large. It is
# not an error, so it is reported rather than capped.
...
# ── 5. Coverage: what fraction of the true national total is in each cell ────
...
fwrite(cov, out("qwi_coverage.csv"))
...
saveRDS(panel, out("qwi_naics3_age_quarter.rds"), compress = "xz")
...
), out("qwi_meta.json"), auto_unbox = TRUE, pretty = TRUE)

say("")
say("Wrote:")
for (f in c("qwi_us_sector.rds", "qwi_state_naics3_slim.rds",
            "qwi_naics3_age_quarter.rds", "qwi_coverage.csv", "qwi_meta.json")) {
  say("  %-32s %s", f, format(structure(file.info(out(f))$size, class = "object_size"),
                              units = "auto"))
}
