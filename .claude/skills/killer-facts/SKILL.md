...
For multi-entry runs, fan out subagents with each agent handling 3-6 entries. Give each agent this skill file to read, plus its entries' `id`, `title`, `source`, `sourceUrl`, and the tracker `keyFinding` (orientation only, never a quote source). Agents write JSON to scratchpad files; the coordinating session aggregates, dedupes across agents, normalizes rating drift by re-reading a sample from each agent, and merges into `data/killer-facts-data.js` sorted by `paperId`.
