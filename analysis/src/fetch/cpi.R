#!/usr/bin/env Rscript
# ─────────────────────────────────────────────────────────────────────────────
# fetch/cpi.R — CPI-U, All Items, seasonally adjusted (CUSR0000SA0), with a
# December 2022 = 100 base column. The deflator for any real-wage outcome.
#
# Descends from the Yale Budget Lab's pull_cpi.R, differing only in that the key
# comes from ~/.Renviron (BLS_KEY) rather than being pasted into the source.
#
# The October 2025 observation does not exist. The funding lapse stopped
# collection and BLS returns "-" for that month; it is the same gap that removed
# the October 2025 CPS sample. That gap is declared in KNOWN_MISSING and any
# OTHER hole is a hard stop, so a silently truncated API response cannot pass for
# a real series.
#
# Writes  inputs/cps/cpi_monthly.csv   (year, month, cpi, cpi_base)
#
#   Rscript analysis/src/fetch/cpi.R [--out=<dir>]
# ─────────────────────────────────────────────────────────────────────────────
...
# The CPI lives with the CPS because it exists to deflate CPS wages.
dest <- file.path(fetch_dir("cps"), "cpi_monthly.csv")

key <- Sys.getenv("BLS_KEY")
if (!nzchar(key))
  stop("BLS_KEY is not set.\n",
       '  Add a line to ~/.Renviron:  BLS_KEY="your key"\n',
       "  Register at https://data.bls.gov/registrationEngine/", call. = FALSE)

# The v2 API caps a request at 20 years, so this is chunked at 10.
...
if (!length(base)) stop("December 2022 is not in the series; cannot rebase.", call. = FALSE)
cpi$cpi_base <- base

# Sanity: monthly and complete, except for the documented gap. Downstream,
# quarterly CPI is a mean over the months present, so 2025Q4 is deflated on
# November and December alone. That matches the Budget Lab's
# `mean(cpi, na.rm = TRUE)` and is noted on the page.
...
if (any(!known))
  stop(sprintf("Unexplained CPI gaps: %s",
               paste(sprintf("%d-%02d", gaps$year[!known], gaps$month[!known]),
                     collapse = ", ")), call. = FALSE)
