#!/usr/bin/env Rscript
# ─────────────────────────────────────────────────────────────────────────────
# clean/exposure.R — occupational AI exposure, aggregated to industries.
#
#   exposure(j) = sum_o  employment_share(o | j) x exposure(o)
#
# Seven published exposure measures are carried, plus a PCA composite over
# them. The composite is what the site plots; the other six exist so the
# robustness table can show the result does not depend on which measure you
# believe. Aggregation weights are CPS occupation-by-industry employment
# shares pooled over 2021-22.
#
# Pure transform, offline. This is the reuse surface: anything in this
# repository that needs an exposure number reads interim/exposure/, never
# inputs/exposure/.
#
# Reads   inputs/exposure/yale_ai_exposure_occ2010.csv   (sha256-pinned by fetch/exposure.R)
#         inputs/exposure/yale_cyclicality_betas.csv
#         src/lookups/cps_ind_to_naics3.csv
#         frozen/cps_occ_ind_weights.rds                 (clean/cps_industry_weights.R)
# Writes  interim/exposure/exposure_occ2010.csv          tidy long, one row per occ x metric
#         interim/exposure/exposure_naics3.csv           employment-weighted industry exposure
#         interim/exposure/exposure_naics2.csv
#         interim/exposure/exposure_weight_sensitivity.csv
#         interim/exposure/exposure_meta.json
#
...
# Path shims so the transform below reads exactly as it did before the move.
here <- function(...) {
  a <- c(...)
  if (identical(a[1], "raw"))        input_path("exposure", a[-1])
  else if (identical(a[1], "crosswalks")) lookup_path(a[-1])
  else interim_path("exposure", a)
}
...
# ── 2. Industry weights ──────────────────────────────────────────────────────
w_path <- frozen_path("cps_occ_ind_weights.rds")
